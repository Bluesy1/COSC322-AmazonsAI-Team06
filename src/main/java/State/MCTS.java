...
public class MCTS {
    ...
    public MCTS(State initialState, boolean isBlack, int topActions, int moveCounter) {
        ...
        this.root = new Node(initialState, null, isBlack, topActions);
    ...
    private Node expand(Node node) {
        Action[] actions = actionFactory.getAction(node.state, isBlack, 0, topActions);
        for (Action action : actions) {
            if (!node.hasChild(action)) {
                State newState = new State(node.state, action);
                Node child = new Node(newState, action, isBlack, topActions);
                node.addChild(child);
                return child;
            }
        }
    ...
    private class Node {
        ...
        public final AtomicReference<Double> totalScore = new AtomicReference<>(0.0);
        public int topN;
        ...
        public Node selectChild(double explorationWeight) {
            return children.parallelStream()
                    .max(Comparator.comparingDouble(c -> {
                        double score = c.totalScore.get() / c.visits.get();
                        return score + explorationWeight * Math.sqrt(Math.log(visits.get()) / c.visits.get());
                    }))
                    .orElseThrow();
        }
        ...
        public void updateStats(double result) {
            ...
            // Thread-safe double update using compareAndSet
            while (true) {
                double current = totalScore.get();
                double next = current + result;
                if (totalScore.compareAndSet(current, next)) {
                    break;
                }
            }
        }
